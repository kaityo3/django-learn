...
class UserEditForm(forms.ModelForm):
    username = forms.CharField(label="名前")
    age = forms.IntegerField(label="歳",min_value=0)
    email = forms.EmailField(label="メルアド")
    picture = forms.FileField(label="写真アップロード",required=False)
...
class PasswordChangeForm(forms.ModelForm):
    ...
    class Meta():
        model = Users
        # Usersに紐づけられたフィールドを指定する。
        # ここで選択したものが上のフィールドになければそのまま表示される。あれば空で表示される。
        fields = ('password',)
